...
from handloom_runtime.analytic_tracer.utils.utils import *
import logging
...
#     img_orig = image.copy()
#     image[:, :, 0] = cv2.dilate(
#         image[:, :, 0].astype(np.uint8), np.ones((2, 2), dtype=np.uint8)
#     )
#     output = cv2.connectedComponentsWithStats(
#         image[:, :, 0].astype(np.uint8), 8, cv2.CV_32S
#     )
...
def clean_input_color_image(image, start_point):
    ...
    image[:, :, 0] = cv2.dilate(image[:, :, 0].astype(np.uint8), kernel)

    small_img = cv2.pyrDown(image[:, :, 0])

    _, labels, stats, _ = cv2.connectedComponentsWithStats(
        small_img, connectivity=8, ltype=cv2.CV_32S
    )

    labels = cv2.resize(
        labels, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST
    )

    valid_classes = np.flatnonzero(stats[:, cv2.CC_STAT_AREA] > 25)

    valid_mask = np.isin(labels, valid_classes).astype(np.uint8)

    return img_orig * valid_mask[:, :, np.newaxis]
...
def is_valid_successor(
    pt, next_pt, depth_img, color_img, pts, pts_explored_set, cur_dir, lenient=False
):
    pt = np.asarray(pt).squeeze().reshape(-1)[:2].astype(float)
    next_pt = np.asarray(next_pt).squeeze().reshape(-1)[:2].astype(float)
    ...
    no_black_on_path = (
        black_on_path(color_img, pt, next_pt, dilate=False) <= 0.3
        if not lenient
        else 0.6
    )
...
def _flat_pt(p):
    p = np.asarray(p).squeeze().reshape(-1)
    return p[:2].astype(float)


def is_similar(pt, next_pt_1, next_pt_2):
    pt = _flat_pt(pt)
    next_pt_1 = _flat_pt(next_pt_1)
    next_pt_2 = _flat_pt(next_pt_2)

    cos_angle = np.dot(normalize(pt - next_pt_1), normalize(pt - next_pt_2))
    return (
        cos_angle > COS_THRESH_SIMILAR
        and abs(np.linalg.norm(pt - next_pt_1) - np.linalg.norm(pt - next_pt_2)) < 1
        or cos_angle > (1 * 1 + COS_THRESH_SIMILAR) / 2
    )


def dedup_candidates_old(
    pt, candidates, depth_img, color_img, pts, pts_explored_set, cur_dir
):
        ...
        for tier in range(len(candidates)):
            ...
            for i in range(len(cur_candidates)):
                ...
                if (
                    ...
                    and len(filtered_candidates)
                    >= num_pts_to_consider_before_ret / counter
                ):
                    return filtered_candidates
    return filtered_candidates
...
def step_path(image, start_point, points_explored, points_explored_set):
    ...
    cur_point = _flat_pt(start_point)
    points_explored = [_flat_pt(p) for p in points_explored]

    # points_explored should have at least one point
    cur_dir = (
        normalize(start_point - points_explored[-1])
        if len(points_explored) >= NUM_POINTS_BEFORE_DIR
        else None
    )
    ...
    for ss in STEP_SIZES:
        candidates.append(
            cur_point + np.array([dx, dy]).T * ss * image.shape[1] / IDEAL_IMG_DIM
        )
...
def is_too_similar(new_path, existing_paths):
    ...
    new_path = np.array([_flat_pt(p) for p in new_path], dtype=float)
    ...
    def length_index(lst, lns, lst_cumsum=None):
        ...
        # interpolate
        pcts = (lns[:] - distances_cumsum[i - 1]) / (
            distances_cumsum[i] - distances_cumsum[i - 1]
        )
    ...
    for pth in existing_paths:
        pth = pth[0]
        path = np.array([_flat_pt(p) for p in pth], dtype=float)
        ...
        if np.linalg.norm(lns_indx[-1] - lns_indx_new[-1]) < 2.5:
            if np.max(np.linalg.norm(lns_indx - lns_indx_new, axis=-1)) < 4.5:
                ...
                if abs(len(path) - len(new_path)) < 2:
                    ...
                    if (
                        np.linalg.norm(
                            np.array(path[: min_len - 2])
                            - np.array(new_path[: min_len - 2]),
                            axis=-1,
                        ).sum()
                        == 0
                    ):
...
def get_updated_traversed_set(prev_set, prev_point, new_point, copy=True, sidelen=4):
    prev_point = _flat_pt(prev_point)
    new_point = _flat_pt(new_point)
    ...
    if travel_norm == 0:
        tp_to_add = (int(round(prev_point[0])), int(round(prev_point[1])))
        set_cpy[tp_to_add] = 0
        return set_cpy

    for t in range(0, int(travel_norm), sidelen):
        base_pt = prev_point + travel_vec * (t / travel_norm)
        for i in range(-sidelen // 2, sidelen // 2 + 1):
            for j in range(-sidelen // 2, sidelen // 2 + 1):
                pt = base_pt + np.array([i, j], dtype=float)
                tp_to_add = (int(round(pt[0])), int(round(pt[1])))
                set_cpy[tp_to_add] = 0
...
def trace(
    ...
    start_point_1,
    start_point_2,
    stop_when_crossing=False,
    resume_from_edge=False,
    timeout=30,
    bboxes=[],
    viz=False,
    exact_path_len=None,
    ...
    while len(active_paths) > 0:
        ...
        if (
            ...
            and np.min(
                np.linalg.norm(endpoints - active_paths[0][0][-1][None, :], axis=-1)
            )
            < 20
        ...
        step_path_res = step_path(
            image, cur_active_path[0][-1], cur_active_path[0][:-1], cur_active_path[1]
        )
        ...
        if len(step_path_res) == 0:
            ...
            for new_point_idx, new_point in enumerate(reversed(step_path_res)):
                keep_path = not is_too_similar(
                    cur_active_path[0] + [new_point], active_paths[:num_active_paths]
                )
                if keep_path:
        ...
        for i in range(side_len):
            for j in range(side_len_2):
                ...
                if i * side_len + j < len(finished_paths):
                    ...
                    axs[i, j].imshow(
                        visualize_path(image, finished_paths[i * side_len + j])
                    )
