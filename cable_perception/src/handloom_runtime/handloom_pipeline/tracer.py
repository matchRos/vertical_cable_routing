...
from handloom_runtime.model_training.src.model import KeypointsGauss
from handloom_runtime.model_training.config import *
from handloom_runtime.analytic_tracer import simple_uncertain_trace_single
import time
import datetime


MODEL_PATH = Path(__file__).resolve().parents[1] / "models" / "tracer" / "tracer_model.pth"
...
class Tracer:
    ...
    def trace(
        ...
        # prev_pixels (list of N pixel points (y,x) in image row/col order — see CableTracer)
        ...
        need = self.trace_config.condition_len
        if len(starting_points) < need:
            # _get_evenly_spaced_points walks consecutive indices and only accepts steps
            # whose spacing is in (cond_dist, 2*cond_dist). Sparse seeds (e.g.
            # auto_white_rings_from_clip: three ring snaps) are often too far apart,
            # so the walk never collects enough points. Fall back to the first N
            # cable-centered seeds — same idea as AnalyticTracer.trace when len >= need.
            flat = []
            for j in range(len(pixels)):
                cur = np.asarray(pixels[j]).squeeze().reshape(-1)[:2]
                flat.append(cur.astype(float))
            flat_arr = np.array(flat)
            if len(flat_arr) >= need:
                starting_points = flat_arr[:need]
            else:
                raise Exception(
        ...
        # Integer indices for numpy slices (e.g. get_crop_and_cond_pixels). The sparse
        # fallback above uses float64; center_of_crop must not be float for img[a:b].
        starting_points = np.asarray(starting_points, dtype=np.int64)
...
class AnalyticTracer(Tracer):
    ...
    def trace(self, img, prev_pixels, endpoints=None, path_len=20, viz=False, idx=0):
        ...
        prev_pixels = np.array([_flat_pt(p) for p in prev_pixels], dtype=float)

        # If caller already provided enough ordered points, use them directly.
        if len(prev_pixels) >= self.trace_config.condition_len:
            starting_points = prev_pixels[: self.trace_config.condition_len]
        else:
            pixels = self.center_pixels_on_cable(img, prev_pixels)

            for j in range(len(pixels)):
                cur_pixel = np.asarray(pixels[j]).squeeze().reshape(-1)
                if (
                    cur_pixel[0] >= 0
                    and cur_pixel[1] >= 0
                    and cur_pixel[1] < img.shape[1]
        ...
        spline = np.array([_flat_pt(p) for p in spline], dtype=float)
        return spline, trace_end
