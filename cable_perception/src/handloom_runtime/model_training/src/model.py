...
from handloom_runtime.model_training.src.resnet_dilated import (
    Resnet34_8s,
    Resnet50_8s,
    Resnet34_Classifier,
    Resnet50_Classifier,
    ...
    def __init__(
        ...
        if resnet_type == "34":
            self.resnet = Resnet34_8s(
                channels=channels, pretrained=pretrained
            )  # Resnet50_8s(channels=channels, pretrained=True) #Resnet34_8s(channels=channels, pretrained=False)
        elif resnet_type == "50":
...
class ClassificationModel(nn.Module):
    def __init__(
        self, num_classes, img_height=480, img_width=640, resnet_type="34", channels=3
    ):
        ...
        self.resnet = None
        if resnet_type == "34":
            self.resnet = Resnet34_Classifier(
                channels=channels, num_classes=self.num_classes, pretrained=False
            )  # Resnet50_8s(channels=channels, pretrained=True) #Resnet34_8s(channels=channels, pretrained=False)
        elif resnet_type == "50":
            self.resnet = Resnet50_Classifier(
                channels=channels, num_classes=self.num_classes, pretrained=False
            )
        self.sigmoid = torch.nn.Sigmoid()
